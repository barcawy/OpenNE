...
class _CLL(object):
    ...
    def __init__(self, graph, rep_size=128, batch_size=1000, negative_ratio=5, order=3):
        ...
        self.batch_size = batch_size
        self.negative_ratio = negative_ratio
        self.partition = community_louvain.best_partition(graph) 
        self.gen_sampling_table() # negative sampling
        self.sess = tf.Session()
        cur_seed = random.getrandbits(32)
        initializer = tf.contrib.layers.xavier_initializer( # init weight
            uniform=False, seed=cur_seed)
    ...
    def build_graph(self):
        ...
        #self.t_e_context = tf.nn.embedding_lookup(self.context_embeddings, self.t)
        #self.second_loss = -tf.reduce_mean(tf.log_sigmoid(self.sign*tf.reduce_sum(tf.multiply(self.h_e, self.t_e_context), axis=1)))
        self.loss = -tf.reduce_mean(tf.log_sigmoid(self.sign*tf.reduce_sum(tf.multiply(self.h_e, self.t_e), axis=1)))
        optimizer = tf.train.AdamOptimizer(0.001)
        self.train_op = optimizer.minimize(self.loss)
    ...
    def batch_iter(self):
        ...
        edges = [(look_up[x[0]], look_up[x[1]]) for x in self.g_ret.edges()]

        data_size = self.g_ret.number_of_edges()
        edge_set = set([x[0]*numNodes+x[1] for x in edges])
        shuffle_indices = np.random.permutation(np.arange(data_size))
    ...
    def induced_weight_graph(self):
        alpha = 3
        part = self.partition
        self.g_ret.add_nodes_from(part[0].keys())
        
        level = len(part)
        for i in range(level):
            w = pow(alpha, level - 1 - i)
            v = {}
            l = []
            for key, value in part[i].items():
                v.setdefault(value, []).append(key)
            for key, value in v.items():
                l.extend(list(combinations(value, 2)))
            for l_edge in l:
                w_prec = self.g_ret.get_edge_data(l_edge[0], l_edge[1], {'weight': 0}).get('weight', 0)
                self.g_ret.add_edge(l_edge[0], l_edge[1], **{'weight': w_prec + w})
    ...
    def gen_sampling_table(self):
        ...
        """
        modify cll data_edge
        """
        self.induced_weight_graph()
        data_size = self.g_ret.number_of_edges()
        self.edge_alias = np.zeros(data_size, dtype=np.int32)
        self.edge_prob = np.zeros(data_size, dtype=np.float32)
        large_block = np.zeros(data_size, dtype=np.int32)
        ...
        total_sum = sum([self.g_ret[edge[0]][edge[1]]["weight"]
                         for edge in self.g_ret.edges()])
        norm_prob = [self.g_ret[edge[0]][edge[1]]["weight"] *
                     data_size/total_sum for edge in self.g_ret.edges()]
        num_small_block = 0
        num_large_block = 0
        cur_small_block = 0
        cur_large_block = 0
...
class CLL(object):
    ...
    def __init__(self, graph, rep_size=128, batch_size=1000, epoch=10, negative_ratio=5, order=3, label_file=None, clf_ratio=0.5, auto_save=True):
        ...
        if order == 3:
            ...
            """
            self.model1 = _CLL(graph, rep_size/2, batch_size,
                                negative_ratio, order=2)
    ...
    def get_embeddings(self):
        ...
        self.vectors = self.model1.get_embeddings()
